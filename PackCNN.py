...
def homo_relu(ciphertext, scale, degree, cryptoContext):
    ...
    return result
def get_weight_bias(layer, state_dict, index, eps=1e-5):
    if layer == "":
        W = state_dict[f"{layer}conv{index}.weight"]
    elif layer=="downsample0":
        W=state_dict[f"layer2.0.downsample.0.weight"]
    elif layer=="downsample1":
        W=state_dict[f"layer3.0.downsample.0.weight"]
    else:
        W = state_dict[f"{layer}.conv{index}.weight"]
    Weight1 = np.empty((W.shape[0], W.shape[1], W.shape[2], W.shape[3]))
    Bias1 = np.empty((W.shape[0],))
    if layer == "":
        A = state_dict[f"{layer}bn{index}.weight"] / torch.sqrt(state_dict[f"{layer}bn{index}.running_var"] + eps)
        b = -(state_dict[f"{layer}bn{index}.weight"] * state_dict[f"{layer}bn{index}.running_mean"] / torch.sqrt(
            state_dict[f"{layer}bn{index}.running_var"] + eps)) + state_dict[f"{layer}bn{index}.bias"]
    elif layer=="downsample0":
        ...
        for j in range(W.shape[1]):
            for p in range(W.shape[2]):
                for q in range(W.shape[3]):
                    Weight1[i, j, p, q] = W[i, j, p, q].item() * A[i]
    for i in range(W.shape[0]):
        Bias1[i] = b[i].item()
    return Weight1, Bias1
def read_image(batch_size):
    filePath = "/test/cifar10/test_batch.bin"
    IMAGE_SIZE = 3072
    LABEL_SIZE = 1
...
def pre_weight(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, index):
    ...
    weight101, bias101 = get_weight_bias(layer, state_dict, index)
...
def pre_weight2(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, index):
    ...
    if layer=="layer2.0":
        ...
        bias1=np.array(bias101[:16])
        bias2=np.array(bias101[16:])
    if layer=="layer3.0":
        arr_expanded1=arr_expanded101[:32]
        arr_expanded2=arr_expanded101[32:]   
        bias1=np.array(bias101[:32])
        bias2=np.array(bias101[32:])
    pad = min_padding_to_next_multiple_of_k(height, 3)
    height_pad = height + pad
    width_pad = width + pad
    block_num = int(height_pad * width_pad / (wi * wo))  
    ...
    for i in range(group_num):
        ...
        if i < block_num % group_num:
            num += 1
        for j in range(output_channel):
            pre_bias_list11[i][j * temp + num * batch_size:(j + 1) * temp] = 0
    for i in range(group_num):
        num = int(np.floor(block_num / group_num))
        if i < block_num % group_num:
            num += 1
        for j in range(output_channel):
            pre_bias_list22[i][j * temp + num * batch_size:(j + 1) * temp] = 0
    return pre_weight_list,pre_weight_list2, pre_bias_list11, pre_bias_list22
...
rot_sum2 = 0
@fhe.utils.profile_python_function
def conv_downsampling(input,weight,bias,pre_bias_list2,pre_bias_list3,pre_bias_list4,cryptoContext,openfhe_context,height,width,wi,wo,batch_size,in_channel,output_channel):
    N = 65536
    slots = int(N / 2)
    global rot_sum, rot_sum2, test_num
    num_in_cipher = int(slots / (output_channel * batch_size))
    repeat = int(output_channel / in_channel)  
    giant,baby = perfect_square_split(in_channel)  
    # print("baby,giant:", baby, giant)
    temp = batch_size * num_in_cipher  
    pad = min_padding_to_next_multiple_of_k(height, 3)
    height_pad = height + pad
    width_pad = width + pad
    ...
    def conv2(o, p, q, r, input, weight, cryptoContext):
        global rot_sum, rot_sum2, encode_num, test_num
        ...
        if cryptoContext.config.SAVE_MIDDLE == False:
            for i in range(baby):
                for j in range(giant//2):
                    name = f"downsample_layer{o}_{p}_{q}_{r}_{i}_{j}"
    ...
    for i in range(group_num):
        for j in range(wi):
            for k in range(wo):
                output[i][j][k]=conv2(i,j,k,0,input_rotate_list[i][j][k],weight[0][0],cryptoContext)
    for i in range(group_num):
        for j in range(wi):
            for k in range(wo):
...
def conv_batch2(input, zero_num_temp, pre_weight_list, pre_weight_list_edge, edge_list, pre_weight_list2,
            ...
            for j in range(wo):
                            ...
                            if edge_list[i][2][0] != 0:
                                ...
                                temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][1][2],
                                                     pre_weight_list[
                                                         ...
                                                         max(edge_list[i][2][0], 0)][2][0],
                                                     cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][0], pre_weight_list[0][0][1][1],
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][1], pre_weight_list[0][0][1][2],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][0], pre_weight_list[0][0][2][1],
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][1], pre_weight_list[0][0][2][2],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                            ...
                            if edge_list[i][0][0] != 0:
                                ...
                                temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][0][1]][2][2],
                                                     pre_weight_list[
                                                         ...
                                                         max(edge_list[i][0][0], 0)][0][2],
                                                     cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][0], pre_weight_list[0][0][1][0],
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][1], pre_weight_list[0][0][1][1],
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][2], pre_weight_list[0][0][1][2],
                                                 cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][0], pre_weight_list[0][0][2][0],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][1], pre_weight_list[0][0][2][1],
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][2], pre_weight_list[0][0][2][2],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                ...
                if j == (((wi + 1) / 2) - 1):
                    for k in range(wi):
                            ...
                            if edge_list[i][2][0] != 0:
                                ...
                                temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][2][2],
                                                     pre_weight_list[
                                                         ...
                                                         max(edge_list[i][2][0], 0)][2][0],
                                                     cryptoContext)  
                            temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], pre_weight_list[0][0][0][1],
                                                 cryptoContext)  
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], pre_weight_list[0][0][0][2],
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], pre_weight_list[0][0][1][1],
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], pre_weight_list[0][0][1][2],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][0], pre_weight_list[0][0][2][1],
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][1], pre_weight_list[0][0][2][2],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                        ...
                        if k == 1:
                            temp_list = np.empty((9), dtype=object)
                            temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][0], pre_weight_list[0][0][0][0],
                                                 cryptoContext)  
                            temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][1], pre_weight_list[0][0][0][1],
                                                 cryptoContext)  
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][2], pre_weight_list[0][0][0][2],
                                                 cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][0], pre_weight_list[0][0][1][0],
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][1], pre_weight_list[0][0][1][1],
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][2], pre_weight_list[0][0][1][2],
                                                 cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][0], pre_weight_list[0][0][2][0],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][1], pre_weight_list[0][0][2][1],
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][2], pre_weight_list[0][0][2][2],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                        ...
                        if k == wi - 1:
                            ...
                            if edge_list[i][5][0] != 0:
                                ...
                                temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][5][1]][2][0],
                                                     0,
                                                     cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][1], 0,
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][2], 0,
                                                 cryptoContext)  
                            if edge_list[i][3][0] != 0:
                                temp_list[5] = conv2(i, j, k, 5,
                                                     input_rotate_list2[edge_list[i][3][1]][0][0][edge_list[i][3][0]],
                                ...
                                temp_list[8] = conv2(i, j, k, 8, input_rotate_list[edge_list[i][3][1]][1][0],
                                                     0,
                                                     cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][1], 0,
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][2], 0,
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                        ...
                        if k == 0:
                            ...
                            if edge_list[i][2][0] != 0:
                                ...
                                temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][1][2],
                                                     0,
                                                     cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][0], 0,
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][1], 0,
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][0], 0,
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][1], 0,
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                        ...
                        if k == 1:
                            ...
                            if edge_list[i][0][0] != 0:
                                ...
                                temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][0][1]][2][2],
                                                     0,
                                                     cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][0], 0,
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][1], 0,
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][2], 0,
                                                 cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][0], 0,
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][1], 0,
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][2], 0,
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                        ...
                        if k == wi - 1:
                            ...
                            if edge_list[i][3][0] != 0:
                                ...
                                temp_list[8] = conv2(i, j, k, 8, input_rotate_list[edge_list[i][3][1]][2][0],
                                                     0,
                                                     cryptoContext)  
                            temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][1], 0,
                                                 cryptoContext)  
                            temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][2], 0,
                                                 cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][1], 0,
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][2], 0,
                                                 cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][1], 0,
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][2], 0,
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
    ...
    for i in range(group_num):
        encode_temp = fhe.encode(pre_bias_list[i], "", cryptoContext.L - output_cipher[i][0][0].cur_limbs, slots, False,
                                 cryptoContext)
        output_cipher[i][0][0] = fhe.homo_add_pt(output_cipher[i][0][0], encode_temp, cryptoContext)
        output_cipher[i][0][1] = fhe.homo_add_pt(output_cipher[i][0][1], encode_temp, cryptoContext)
        output_cipher[i][1][0] = fhe.homo_add_pt(output_cipher[i][1][0], encode_temp, cryptoContext)
        output_cipher[i][1][1] = fhe.homo_add_pt(output_cipher[i][1][1], encode_temp, cryptoContext)
        output_cipher[i][0][2] = fhe.homo_add_pt(output_cipher[i][0][2], fhe.encode(pre_bias_list2[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][1][2] = fhe.homo_add_pt(output_cipher[i][1][2], fhe.encode(pre_bias_list2[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][2][0] = fhe.homo_add_pt(output_cipher[i][2][0], fhe.encode(pre_bias_list3[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][2][1] = fhe.homo_add_pt(output_cipher[i][2][1], fhe.encode(pre_bias_list3[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][2][2] = fhe.homo_add_pt(output_cipher[i][2][2], fhe.encode(pre_bias_list4[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
...
def conv_batch2_mod2(input, zero_num_temp, pre_weight_list, pre_weight_list_edge, edge_list, pre_weight_list2,
                                ...
                                temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][2][2],
                                                     pre_weight_list[
                                                         ...
                                                         max(edge_list[i][2][0], 0)][2][0]*
                                                                   pre_weight_list4[edge_list[i][2][1]][2][0],
                                                     cryptoContext)  
                            temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], pre_weight_list[0][0][0][1]* pre_weight_list4[i][0][1],
                                                 cryptoContext)  
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], pre_weight_list[0][0][0][2]* pre_weight_list4[i][0][2],
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], pre_weight_list[0][0][1][1]* pre_weight_list4[i][1][1],
                                                 cryptoContext)  
                            temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], pre_weight_list[0][0][1][2]* pre_weight_list4[i][1][2],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][0], pre_weight_list[0][0][2][1]* pre_weight_list4[i][2][1],
                                                 cryptoContext)  
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][1], pre_weight_list[0][0][2][2]* pre_weight_list4[i][2][2],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
    ...
    for i in range(group_num):
        encode_temp = fhe.encode(pre_bias_list[i], "", cryptoContext.L - output_cipher[i][0][0].cur_limbs, slots, False,
                                 cryptoContext)
        output_cipher[i][0][0] = fhe.homo_add_pt(output_cipher[i][0][0], encode_temp, cryptoContext)
        output_cipher[i][1][0] = fhe.homo_add_pt(output_cipher[i][1][0], fhe.encode(pre_bias_list3[i], "",
                                                                            cryptoContext.L -
                                                                            output_cipher[i][0][0].cur_limbs,
                                                                            slots, False, cryptoContext),
                                            cryptoContext)
        ...
        output_cipher[i][0][2] = fhe.homo_add_pt(output_cipher[i][0][2], fhe.encode(pre_bias_list2[i], "",
                                                                                    ...
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][0][1] = fhe.homo_add_pt(output_cipher[i][0][1], fhe.encode(pre_bias_list2[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][1][2] = fhe.homo_add_pt(output_cipher[i][1][2], fhe.encode(pre_bias_list4[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][1][1] = fhe.homo_add_pt(output_cipher[i][1][1], fhe.encode(pre_bias_list4[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
        output_cipher[i][2][1] = fhe.homo_add_pt(output_cipher[i][2][1], fhe.encode(pre_bias_list4[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                 cryptoContext)
            ...
            for l in range(len(down_need[i])):
                    ...
                    if k == wi - 1:
                        ...
                        if edge_list[i][5][0] != 0:
                            ...
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][5][1]][2][0],
                                                 ...
                                                 pre_weight_list3a[edge_list[i][5][1]][0][2],
                                                 cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][1], pre_weight_list_edgea[i][1][0],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][2], pre_weight_list_edgea[i][1][1],
                                             cryptoContext)  
                        if edge_list[i][3][0] != 0:
                            temp_list[5] = conv2(i, j, k, 5,
                                                 input_rotate_list2[edge_list[i][3][1]][0][0][edge_list[i][3][0]],
                            ...
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[edge_list[i][3][1]][1][0],
                                                 ...
                                                 pre_weight_list3a[edge_list[i][3][1]][2][2],
                                                 cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][1], pre_weight_list_edgea[i][2][0],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][2], pre_weight_list_edgea[i][2][1],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                        ...
                        if edge_list[i][2][0] != 0:
                            ...
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][1][2],
                                                 pre_weight_lista[
                                                     ...
                                                     max(edge_list[i][2][0], 0)][2][0],
                                                 cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][0], pre_weight_lista[0][0][1][1],
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][1], pre_weight_lista[0][0][1][2],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][0], pre_weight_lista[0][0][2][1],
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][1], pre_weight_lista[0][0][2][2],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                        ...
                        if edge_list[i][0][0] != 0:
                            ...
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][0][1]][2][2],
                                                 pre_weight_lista[
                                                     ...
                                                     max(edge_list[i][0][0], 0)][0][2],
                                                 cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][0], pre_weight_lista[0][0][1][0],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][1], pre_weight_lista[0][0][1][1],
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][2], pre_weight_lista[0][0][1][2],
                                             cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][0], pre_weight_lista[0][0][2][0],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][1], pre_weight_lista[0][0][2][1],
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][2], pre_weight_lista[0][0][2][2],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                ...
                if j == (((wi + 1) / 2) - 1):
                    if k == wi - 1:
                        ...
                        if edge_list[i][3][0] != 0:
                            ...
                            temp_list[8] = conv2(i, j, k, 8, input_rotate_list[edge_list[i][3][1]][2][0],
                                                 ...
                                                 pre_weight_list3a[edge_list[i][3][1]][2][2],
                                                 cryptoContext)  
                        temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][1], pre_weight_list_edgea[i][0][0],
                                             cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][2], pre_weight_list_edgea[i][0][1],
                                             cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][1], pre_weight_list_edgea[i][1][0],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][2], pre_weight_list_edgea[i][1][1],
                                             cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][1], pre_weight_list_edgea[i][2][0],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][2], pre_weight_list_edgea[i][2][1],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                        ...
                        if edge_list[i][2][0] != 0:
                            ...
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][2][2],
                                                 pre_weight_lista[
                                                     ...
                                                     max(edge_list[i][2][0], 0)][2][0],
                                                 cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], pre_weight_lista[0][0][0][1],
                                             cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], pre_weight_lista[0][0][0][2],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], pre_weight_lista[0][0][1][1],
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], pre_weight_lista[0][0][1][2],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][0], pre_weight_lista[0][0][2][1],
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][1], pre_weight_lista[0][0][2][2],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                    ...
                    if k == 1:
                        temp_list = np.empty((9), dtype=object)
                        temp_list1 = np.empty((9), dtype=object)
                        temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][0], pre_weight_lista[0][0][0][0],
                                             cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][1], pre_weight_lista[0][0][0][1],
                                             cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][2], pre_weight_lista[0][0][0][2],
                                             cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][0], pre_weight_lista[0][0][1][0],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][1], pre_weight_lista[0][0][1][1],
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][2], pre_weight_lista[0][0][1][2],
                                             cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][0], pre_weight_lista[0][0][2][0],
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][1], pre_weight_lista[0][0][2][1],
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][2], pre_weight_lista[0][0][2][2],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
            ...
            for l in range(len(down_need[i])):
                ...
                if j == (((wi + 1) / 2) - 1):
                    ...
                    if k == 0:
                        ...
                        if edge_list[i][2][0] != 0:
                            ...
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][2][2],
                                                 0,
                                                 cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], 0,
                                             cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], 0,
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], 0,
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], 0,
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][0], 0,
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][1], pre_weight_lista[0][0][2][2],
                                             cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                    ...
                    if k == 1:
                        temp_list = np.empty((9), dtype=object)
                        temp_list1 = np.empty((9), dtype=object)
                        temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][0], 0,
                                             cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][1], 0,
                                             cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][2], 0,
                                             cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][0], 0,
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][1], 0,
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][2], 0,
                                             cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][0], 0,
                                             cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][1], pre_weight_lista[0][0][2][1],
                                             cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][2], pre_weight_lista[0][0][2][2],
        ...
        for i in range(group_num):
            for j in range(wi):
                for k in range(wo):
                    ...
                    output_cipher[i][j][k] = fhe.homo_add(output_cipher[i][j][k], res_initial[i][j][k], cryptoContext)
    for i in range(group_num):
        encode_temp = fhe.encode(pre_bias_list[i], "", cryptoContext.L - output_cipher[0][0][0].cur_limbs, slots, False,
                                 cryptoContext)
        for l in range(len(down_need[i])):
            j, k = down_need[i][l]
            if j==0 and k==0:
            ...
            if j == 2 and k == 2:
                output_cipher[i][2][2] = fhe.homo_add_pt(output_cipher[i][2][2], fhe.encode(pre_bias_list4[i], "",
                                                     ...
                                                     cryptoContext)
    for i in range(group_num):
        encode_temp = fhe.encode(pre_bias_lista[i], "", cryptoContext.L - output_cipher[0][0][0].cur_limbs, slots, False,
                                 cryptoContext)
        for l in range(len(down_need[i])):
            j, k = down_need[i][l]
            if j==0 and k==0:
                output_cipher[i+group_num][0][0] = fhe.homo_add_pt(output_cipher[i+group_num][0][0], encode_temp, cryptoContext)
            if j == 0 and k == 1:
                output_cipher[i+group_num][0][1] = fhe.homo_add_pt(output_cipher[i+group_num][0][1], encode_temp, cryptoContext)
            if j == 1 and k == 0:
                output_cipher[i+group_num][1][0] = fhe.homo_add_pt(output_cipher[i+group_num][1][0], encode_temp, cryptoContext)
            if j == 1 and k == 1:
                output_cipher[i+group_num][1][1] = fhe.homo_add_pt(output_cipher[i+group_num][1][1], encode_temp, cryptoContext)
            if j == 0 and k == 2:
                output_cipher[i+group_num][0][2] = fhe.homo_add_pt(output_cipher[i+group_num][0][2], fhe.encode(pre_bias_list2a[i], "",
                                                                                        cryptoContext.L -
                                                                                        output_cipher[0][0][0].cur_limbs,
                                                                                        slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 1 and k == 2:
                output_cipher[i+group_num][1][2] = fhe.homo_add_pt(output_cipher[i+group_num][1][2], fhe.encode(pre_bias_list2a[i], "",
                                                                                        cryptoContext.L -
                                                                                        output_cipher[0][0][0].cur_limbs,
                                                                                        slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 2 and k == 0:
                output_cipher[i+group_num][2][0] = fhe.homo_add_pt(output_cipher[i+group_num][2][0], fhe.encode(pre_bias_list3a[i], "",
                                                                                        cryptoContext.L -
                                                                                        output_cipher[0][0][0].cur_limbs,
                                                                                        slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 2 and k == 1:
                output_cipher[i+group_num][2][1] = fhe.homo_add_pt(output_cipher[i+group_num][2][1], fhe.encode(pre_bias_list3a[i], "",
                                                                                        cryptoContext.L -
                                                                                        output_cipher[0][0][0].cur_limbs,
                                                                                        slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 2 and k == 2:
                output_cipher[i+group_num][2][2] = fhe.homo_add_pt(output_cipher[i+group_num][2][2], fhe.encode(pre_bias_list4a[i], "",
                                                                                        cryptoContext.L -
                                                                                        output_cipher[0][0][0].cur_limbs,
                                                                                        slots, False, cryptoContext),
                                                     cryptoContext)
            ...
            for l in range(len(down_need[i])):
                ...
                if j == (((wi + 1) / 2) - 1):
                    ...
                    if k == 0:
                        ...
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], pre_weight_list[0][0][0][1]* pre_weight_list4[i][0][1],
                                             cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], pre_weight_list[0][0][0][2]* pre_weight_list4[i][0][1],
                                             cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], pre_weight_list[0][0][1][1]* pre_weight_list4[i][1][1],
                                             cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], pre_weight_list[0][0][1][2]* pre_weight_list4[i][1][2],
            ...
            for l in range(len(down_need[i])):
                            ...
                            temp_list[2] = conv2(i, j, k, 2, input_rotate_list[edge_list[i][0][1]][2][2],
                                                pre_weight_lista[
                                                    ...
                                                    max(edge_list[i][0][0], 0)][0][2]*
                                                            pre_weight_list2a[edge_list[i][0][1]][0][2],
                                                cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][0][0], pre_weight_list_edgea[i][1][0],
                                            cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][0][1],pre_weight_list_edgea[i][1][1],
                                            cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][0][2], pre_weight_list_edgea[i][1][2],
                                            cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][1][0], pre_weight_list_edgea[i][2][0],
                                            cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][1][1], pre_weight_list_edgea[i][2][1],
                                            cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][1][2], pre_weight_list_edgea[i][2][2],
                                            cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                ...
                if j == (((wi + 1) / 2) - 1):
                    if k == wi - 1:
                            ...
                            if edge_list[i][3][0] != 0:
                                ...
                                temp_list[8] = conv2(i, j, k, 8, input_rotate_list[edge_list[i][3][1]][2][0],
                                                     ...
                                                     pre_weight_list4a[edge_list[i][3][1]][2][2],
                                                     cryptoContext)  
                            temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][1], pre_weight_list_edgea[i][0][0]* pre_weight_list4a[i][0][0],
                                                 cryptoContext)  
                            temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][2], pre_weight_list_edgea[i][0][1]* pre_weight_list4a[i][0][1],
                                                 cryptoContext)  
                            temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][1], pre_weight_list_edgea[i][1][0]* pre_weight_list4a[i][1][0],
                                                 cryptoContext)  
                            temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][2], pre_weight_list_edgea[i][1][1]* pre_weight_list4a[i][1][1],
                                                 cryptoContext)  
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][1], pre_weight_list_edgea[i][2][0]* pre_weight_list4a[i][2][0],
                                                 cryptoContext)  
                            temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][2], pre_weight_list_edgea[i][2][1]* pre_weight_list4a[i][2][1],
                                                 cryptoContext)  
                            output_giant_ = np.empty(baby, dtype=object)
                            for x in range(baby):
                                for y in range(9):
                            ...
                            temp_list[6] = conv2(i, j, k, 6, input_rotate_list[edge_list[i][2][1]][2][2],
                                                    pre_weight_lista[
                                                        ...
                                                        max(edge_list[i][2][0], 0)][2][0]*
                                                                pre_weight_list4a[edge_list[i][2][1]][2][0],
                                                    cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][0], pre_weight_lista[0][0][0][1]* pre_weight_list4a[i][0][1],
                                                cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][1], pre_weight_lista[0][0][0][2]* pre_weight_list4a[i][0][2],
                                                cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][0], pre_weight_lista[0][0][1][1]* pre_weight_list4a[i][1][1],
                                                cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][1], pre_weight_lista[0][0][1][2]* pre_weight_list4a[i][1][2],
                                                cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][0], pre_weight_lista[0][0][2][1]* pre_weight_list4a[i][2][1],
                                                cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][1], pre_weight_lista[0][0][2][2]* pre_weight_list4a[i][2][2],
                                                cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
                    ...
                    if k == 1:
                        temp_list = np.empty((9), dtype=object)
                        temp_list1 = np.empty((9), dtype=object)
                        temp_list[0] = conv2(i, j, k, 0, input_rotate_list[i][0][0], pre_weight_list_edgea[i][0][0] * pre_weight_list4a[i][0][0],
                                                cryptoContext)  
                        temp_list[1] = conv2(i, j, k, 1, input_rotate_list[i][0][1], pre_weight_list_edgea[i][0][1] * pre_weight_list4a[i][0][1],
                                                cryptoContext)  
                        temp_list[2] = conv2(i, j, k, 2, input_rotate_list[i][0][2], pre_weight_list_edgea[i][0][2] * pre_weight_list4a[i][0][2],
                                                cryptoContext)  
                        temp_list[3] = conv2(i, j, k, 3, input_rotate_list[i][1][0], pre_weight_list_edgea[i][1][0] * pre_weight_list4a[i][1][0],
                                                cryptoContext)  
                        temp_list[4] = conv2(i, j, k, 4, input_rotate_list[i][1][1], pre_weight_list_edgea[i][1][1] * pre_weight_list4a[i][1][1],
                                                cryptoContext)  
                        temp_list[5] = conv2(i, j, k, 5, input_rotate_list[i][1][2], pre_weight_list_edgea[i][1][2] * pre_weight_list4a[i][1][2],
                                                cryptoContext)  
                        temp_list[6] = conv2(i, j, k, 6, input_rotate_list[i][2][0], pre_weight_list_edgea[i][2][0] * pre_weight_list4a[i][2][0],
                                                cryptoContext)  
                        temp_list[7] = conv2(i, j, k, 7, input_rotate_list[i][2][1], pre_weight_list_edgea[i][2][1] * pre_weight_list4a[i][2][1],
                                                cryptoContext)  
                        temp_list[8] = conv2(i, j, k, 8, input_rotate_list[i][2][2], pre_weight_list_edgea[i][2][2] * pre_weight_list4a[i][2][2],
                                                cryptoContext)  
                        output_giant_ = np.empty(baby, dtype=object)
                        for x in range(baby):
                            for y in range(9):
        ...
        for l in range(len(down_need[i])):
            ...
            if j == 0 and k == 1:
                output_cipher[i][0][1] = fhe.homo_add_pt(output_cipher[i][0][1], fhe.encode(pre_bias_list2[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext), cryptoContext)
            if j == 1 and k == 0:
                output_cipher[i][1][0] = fhe.homo_add_pt(output_cipher[i][1][0], fhe.encode(pre_bias_list3[i], "",
                                                                            cryptoContext.L -
                                                                            output_cipher[i][0][0].cur_limbs,
    ...
    for i in range(group_num):
        ...
        for l in range(len(down_need[i])):
            ...
            if j==0 and k==0:
                output_cipher[i+group_num][0][0] = fhe.homo_add_pt(output_cipher[i+group_num][0][0], encode_temp, cryptoContext)
            if j == 0 and k == 1:
                output_cipher[i+group_num][0][1] = fhe.homo_add_pt(output_cipher[i+group_num][0][1], fhe.encode(pre_bias_list2a[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext), cryptoContext)
            if j == 1 and k == 0:
                output_cipher[i+group_num][1][0] = fhe.homo_add_pt(output_cipher[i+group_num][1][0], fhe.encode(pre_bias_list3a[i], "",
                                                                            cryptoContext.L -
                                                                            output_cipher[i][0][0].cur_limbs,
                                                                            slots, False, cryptoContext), cryptoContext)
            if j == 1 and k == 1:
                output_cipher[i+group_num][1][1] = fhe.homo_add_pt(output_cipher[i+group_num][1][1], fhe.encode(pre_bias_list4a[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext), cryptoContext)
            if j == 0 and k == 2:
                output_cipher[i+group_num][0][2] = fhe.homo_add_pt(output_cipher[i+group_num][0][2], fhe.encode(pre_bias_list2a[i], "",
                                                     ...
                                                     cryptoContext)
            if j == 1 and k == 2:
                output_cipher[i+group_num][1][2] = fhe.homo_add_pt(output_cipher[i+group_num][1][2], fhe.encode(pre_bias_list4a[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 2 and k == 0:
                output_cipher[i+group_num][2][0] = fhe.homo_add_pt(output_cipher[i+group_num][2][0], fhe.encode(pre_bias_list3a[i], "",
                                                     ...
                                                     cryptoContext)
            if j == 2 and k == 1:
                output_cipher[i+group_num][2][1] = fhe.homo_add_pt(output_cipher[i+group_num][2][1], fhe.encode(pre_bias_list4a[i], "",
                                                                                    cryptoContext.L -
                                                                                    output_cipher[i][0][0].cur_limbs,
                                                                                    slots, False, cryptoContext),
                                                     cryptoContext)
            if j == 2 and k == 2:
                output_cipher[i+group_num][2][2] = fhe.homo_add_pt(output_cipher[i+group_num][2][2], fhe.encode(pre_bias_list4a[i], "",
                                                     ...
                                                     cryptoContext)
    return output_cipher
def mask_top_or_bottom(pre_weight_list, pre_bias_list, wi, wo, height, width, batch_size, in_channel,
...
def pre_mask2(wi, wo, height, width, batch_size, in_channel, cryptoContext):  
    N = 65536
    slots = int(N / 2)
...
def homo_Aespa_perfect_square(x, filename, cryptoContext):
    ...
    temp=np.ones(slots)
    n1=fhe.encode(temp,"",1,2**15,False,cryptoContext)
    ...
    n2=fhe.encode(temp,"",1,2**15,False,cryptoContext)

    res = fhe.homo_add_pt(perfect_squre, n2, cryptoContext)
    return res

@fhe.utils.profile_python_function
def batch_homo_relu(input, filename, cryptoContext):
    for i in range(input.shape[0]):
        for j in range(input.shape[1]):
            for k in range(input.shape[2]):
                input[i][j][k] = homo_Aespa_perfect_square(input[i][j][k], filename, cryptoContext)
    return input
...
def batch_homo_bs(input, index, logBsSlots_list, levelBudget_list, cryptoContext):
    for i in range(input.shape[0]):
        for j in range(input.shape[1]):
            for k in range(input.shape[2]):
                input[i][j][k] = fhe.homo_bootstrap(input[i][j][k], cryptoContext.L-index, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
    return input
def plaintext_relu(input, scale, openfhe_context):
    for i in range(input.shape[0]):
        for j in range(input.shape[1]):
            for k in range(input.shape[2]):
                temp = openfhe_context.decrypt(input[i][j][k]).cpu().numpy().reshape(-1)
                temp /= scale
                temp2 = relu(torch.tensor(temp))
                temp2 = temp2.numpy()
                input[i][j][k] = openfhe_context.encrypt(temp2, "cuda", 1, 0, 2 ** 15)
    return input
def final_weight(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, left_edge,
                 right_edge, bottom_edge, up_edge, index):
    a, b = pre_weight(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, index)
...
def final_weight2(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, left_edge,
                 right_edge, bottom_edge, up_edge, index):
    a, a2,b,b2 = pre_weight2(batch_size, in_channel, state_dict, output_channel, height, width, wi, wo, layer, scale, index)
    pre_weight_list = mask_top_or_bottom(a, b, wi, wo, height, width, batch_size, in_channel,
                                         output_channel,layer)  
    pre_weight_list1, pre_bias_list1, mask_weight_list1, mask_bias_list1 = pre_weight_edge2(left_edge, a, b, batch_size,
                                                                                            in_channel, output_channel,
...
def averagepool(input,batch_size,height,width,wi,wo,in_channel,output_channel,cryptoContext,openfhe_context):
    ...
    for i in range(1,num_in_cipher):
        output=fhe.homo_add(output,fhe.homo_rotate(output,i*batch_size,cryptoContext),cryptoContext)
    return output

@fhe.utils.profile_python_function
def fc(input_cipher,state_dict,batch_size,height,width,wi,wo,in_channel,output_channel,cryptoContext,openfhe_context):
    N = 65536
    slots = int(N / 2)
    output_channel=16
    ...
    fc_pad=np.repeat(fc_bias,repeat)
    baby=4
    giant=4
    temp=batch_size*num_in_cipher
    ...
    for g in range(giant):
            if g!=0:
                input_temp=fhe.homo_rotate(input_cipher,g*temp,cryptoContext)
            else:
...
def batch_CNN():
    ...
    relu_degree = 180
    slots = 2 ** 15
    loadorsave = 0#set to 0 on the first time; otherwise, set to 1
    if loadorsave == 0:  
        SAVE_MIDDLE = True
        DIRECT_LOAD = False
    else:
    ...
    cryptoContext.weight_path = "/test/weights_aespa_20/"
    ckpt = torch.load("/test/cifar10_resnet20-4118986f.pt", map_location="cpu")
    # ckpt = torch.load("/test/resnet110_cifar10_final.pt", map_location="cpu")
    pre_encode_type = "middle"
    cryptoContext.pre_encode_type = pre_encode_type
    #for the first time:these line needs to be commented out.
    pkl_path = DATA_DIR + "/encode_20260122_204117.pkl"
    load_weight(pkl_path, cryptoContext)
    
    pre_mask = pre_mask2(3, 3, 32, 32, 128, 16, cryptoContext)
    pre_mask22 = pre_mask2(3, 3, 16, 16, 128, 32, cryptoContext)
    ...
    def initial_layer0():
        conv1_mode = "save"
        if conv1_mode == "save":
            pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,_=final_weight(128,4,state_dict,16,32,32,3,3,"",scale,left_edge,right_edge,bottom_edge,up_edge,1)
            ...
            output=batch_homo_relu(output,"conv1bn1", cryptoContext)
            output = handle_modes(conv1_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
                                  "conv1",
                                  "After conv1")
        else:
            output = handle_modes(conv1_mode, None, None, DATA_DIR, "conv1", "After conv1")
        return output
    def layer1(output, block_num):
        for i in range(block_num):
            ...
            if conv2_mode == "save":
                scale = 1
                time112 = time.time()
                pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,_=final_weight(128,16,state_dict,16,32,32,3,3,f"layer1.{i}",scale,left_edge,right_edge,bottom_edge,up_edge,1)
                time11 = time.time()
                time22 = time.time()
                print("load npz time:", time22 - time11)
            ...
            #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
            #                           f"conv{1}_{i * 2}", f"conv{1}_{i * 2}")
            #     print("test_num rotate:", test_num)
            # else:
            #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{1}_{i * 2}", f"conv{1}_{i * 2}")
            output=batch_homo_relu(output,f"layer{i+1}-conv{1}bn{1}", cryptoContext)
            print("conv2 rotate:", rot_sum, rot_sum2)
            if conv2_mode == "save":
                scale = 1
                pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,_=final_weight(128,16,state_dict,16,32,32,3,3,f"layer1.{i}",scale,left_edge,right_edge,bottom_edge,up_edge,2)
                output = conv_batch2(output, zero_num_temp, pre_weight_list, pre_weight_list1, edge_list,
                                     mask_weight_list1,
                                     mask_weight_list2, mask_weight_list3,
                                     mask_weight_list4, b, pre_bias_list1, pre_bias_list2, final_bias, 128, 16, 16, 32,
            ...
            #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
            #                           f"conv{1}_{i * 2 + 1}", f"conv{1}_{i * 2 + 1}")
            # else:
            #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{1}_{i * 2 + 1}", f"conv{1}_{i * 2 + 1}")
            # if i==2:
            #     index=0
            #     output = batch_homo_bs(output, index, logBsSlots_list, levelBudget_list, cryptoContext)
            output=batch_homo_relu(output,f"layer{i+1}-conv{2}bn{2}", cryptoContext)
        ...
        for i in range(block_num):
            ...
            if i==0:
                if conv2_mode == "save":
                    scale = 1
                    time112 = time.time()
                    pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,pre_weight_lista,pre_weight_list1a,mask_weight_list1a,mask_weight_list2a,mask_weight_list3a,mask_weight_list4a,b2,pre_bias_list1a,pre_bias_list2a,final_biasa=final_weight2(128,16,state_dict,16,32,32,3,3,f"layer2.{i}",scale,left_edge,right_edge,bottom_edge,up_edge,1)
                    np.savez(f'/test/pre_layer_weight/layer2.{i}.1.npz',
                             arr1=pre_weight_list, arr2=pre_weight_list1, arr3=mask_weight_list1,
                             arr4=mask_weight_list2, arr5=mask_weight_list3, arr6=mask_weight_list4, arr7=b,
                             arr8=pre_bias_list1, arr9=pre_bias_list2, arr10=final_bias)
                    time11 = time.time()
                ...
                #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
                #                           f"conv{2}_{i * 2}", f"conv{2}_{i * 2}")
                #     print("test_num rotate:", test_num)
                # else:
                #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{2}_{i * 2}", f"conv{2}_{i * 2}")
                output = downsampling(output, pre_mask, 3, 3, 32, 32, 128, 16, 32, 2, cryptoContext, openfhe_context)
                output=batch_homo_relu(output,f"layer{i+4}-conv{1}bn{1}", cryptoContext)
                print("conv2 rotate:", rot_sum, rot_sum2)
            else:
                if conv2_mode == "save":
                    scale = 1
                    ...
                    # output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]],
                    #                       DATA_DIR,
                    #                       f"conv{2}_{i * 2}", f"conv{2}_{i * 2}")
                #     print("test_num rotate:", test_num)
                # else:
                #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{2}_{i * 2}", f"conv{2}_{i * 2}")
                output=batch_homo_relu(output,f"layer{i+4}-conv{1}bn{1}", cryptoContext)
                # afff=openfhe_context.decrypt(output[0][0][0])
                # print("hello world:",afff[0],afff[128],afff[128*5])
            if conv2_mode == "save":
                if i==0:
                    _,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,bias_down,pre_bias_list2,pre_bias_list3,final_bias,weight_down=final_weight(128,32,state_dict,32,16,16,3,3,f"downsample0",scale,left_edge2,right_edge2,bottom_edge2,up_edge2,2)
                    ...
                    templist=conv_downsampling(templist,weight_down,bias_down,pre_bias_list2,pre_bias_list3,final_bias,cryptoContext,openfhe_context,16,16,3,3,128,32,32)
                    # middle_test_cipher=openfhe_context.decrypt(templist[4][2][1])
                scale = 1
                pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,_=final_weight(128,32,state_dict,32,16,16,3,3,f"layer2.{i}",scale,left_edge2,right_edge2,bottom_edge2,up_edge2,2)
                np.savez(f'/test/pre_layer_weight/layer2.{i}.2.npz',
                         arr1=pre_weight_list, arr2=pre_weight_list1, arr3=mask_weight_list1,
                         arr4=mask_weight_list2, arr5=mask_weight_list3, arr6=mask_weight_list4, arr7=b,
                         arr8=pre_bias_list1, arr9=pre_bias_list2, arr10=final_bias)
                output = conv_batch2_mod2(output, zero_num_temp2, pre_weight_list, pre_weight_list1, edge_list2,
            ...
            #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
            #                           f"conv{2}_{i * 2 + 1}", f"conv{2}_{i * 2 + 1}")
            # else:
            #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{2}_{i * 2 + 1}", f"conv{2}_{i * 2 + 1}")
            # if i==0:
            #     index=maxLevelsRemaining - 9
            #     output = batch_homo_bs(output, index, logBsSlots_list, levelBudget_list, cryptoContext)
            ...
            #     index=1
            #     output = batch_homo_bs(output, index, logBsSlots_list, levelBudget_list, cryptoContext)
            output=batch_homo_relu(output,f"layer{i+4}-conv{2}bn{2}", cryptoContext)
            print("conv2 rotate:", rot_sum, rot_sum2)
            print("test_num rotate:", test_num)
        return output
    def layer3(output, block_num):
        for i in range(block_num):
            ...
            if i==0:
                ...
                #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
                #                           f"conv{3}_{i * 2}", f"conv{3}_{i * 2}")
                # else:
                #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{3}_{i * 2}", f"conv{3}_{i * 2}")
                output = downsampling(output, pre_mask22, 3, 3, 16, 16, 128, 32, 64, 2, cryptoContext, openfhe_context)
                output=batch_homo_relu(output,f"layer{i+7}-conv{1}bn{1}", cryptoContext)
            else:
                if conv2_mode == "save":
                    scale = 1
                    time112 = time.time()
            ...
            if conv2_mode == "save":
                if i==0:
                    _,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,bias_down,pre_bias_list2,pre_bias_list3,final_bias,weight_down=final_weight(128,64,state_dict,64,8,8,3,3,f"downsample1",scale,left_edge3,right_edge3,bottom_edge3,up_edge3,2)
                    conv2_test = np.empty((templist.shape[0] * 2, *templist.shape[1:]), dtype=object)
                    conv2_test[:templist.shape[0]] = templist  
                    conv2_test[templist.shape[0]:] = templist  
                    templist=downsampling(conv2_test, pre_mask22, 3, 3, 16, 16, 128, 32, 64, 2, cryptoContext, openfhe_context)
                    templist=conv_downsampling(templist,weight_down,bias_down,pre_bias_list2,pre_bias_list3,final_bias,cryptoContext,openfhe_context,8,8,3,3,128,64,64)
                scale = 1
                pre_weight_list,pre_weight_list1,mask_weight_list1,mask_weight_list2,mask_weight_list3,mask_weight_list4,b,pre_bias_list1,pre_bias_list2,final_bias,_=final_weight(128,64,state_dict,64,8,8,3,3,f"layer3.{i}",scale,left_edge3,right_edge3,bottom_edge3,up_edge3,2)
                np.savez(f'/test/pre_layer_weight/layer3.{i}.2.npz',
                         arr1=pre_weight_list, arr2=pre_weight_list1, arr3=mask_weight_list1,
                         arr4=mask_weight_list2, arr5=mask_weight_list3, arr6=mask_weight_list4, arr7=b,
                         arr8=pre_bias_list1, arr9=pre_bias_list2, arr10=final_bias)
                output = conv_batch2(output, zero_num_temp3, pre_weight_list, pre_weight_list1, edge_list3,
            ...
            #     output = handle_modes(conv2_mode, output, [output.shape[0], output.shape[1], output.shape[2]], DATA_DIR,
            #                           f"conv{3}_{i * 2 + 1}", f"conv{3}_{i * 2 + 1}")
            # else:
            #     output = handle_modes(conv2_mode, None, None, DATA_DIR, f"conv{3}_{i * 2 + 1}", f"conv{3}_{i * 2 + 1}")
            # if i==0:
            #     index=0
            #     output = batch_homo_bs(output, index, logBsSlots_list, levelBudget_list, cryptoContext)
            if i == 0 or i == 3 or i == 6 or i == 9 or i == 12:
            ...
            if i == 15:
                index = 0
                output = batch_homo_bs(output, index, logBsSlots_list, levelBudget_list, cryptoContext)
            output=batch_homo_relu(output,f"layer{i+7}-conv{2}bn{2}", cryptoContext)
        return output
    ...
    # mem("after gc+empty_cache")

    layer0 = initial_layer0()
    # while 1<2:
    #     a=1
    output = layer1(layer0, 18)
    layer2 = layer2(output, 18)
    layer3 = layer3(layer2, 18)
    average=averagepool(layer3,128,8,8,3,3,32,64,cryptoContext,openfhe_context)
    fc1=fc(average,state_dict,128,8,8,3,3,64,10,cryptoContext,openfhe_context)
    # aaa=openfhe_context.decrypt(fc1)
    return 0
